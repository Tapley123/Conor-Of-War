...
public class OldHuman : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            float dist = Vector2.Distance(this.gameObject.transform.position, collision.gameObject.transform.position);
            Debug.Log(dist);
            if (collision.gameObject.GetComponent<Monster>().vampCantAttack == false)
            {
                stopped = true;
                speed = 0;
